...
* シミュレーションを行うと、ロボット等、動く物体のBodyItemの下（あるいはコントローラの下）に “AISTSimulator-XXX” といった名前でBodyMotionアイテムが生成される。これがシミュレーション結果を格納している動作軌道データです。
  
* 上記の各Bodyアイテムを選択状態にして（まとめて選択してもOK), 「ファイル」⇒「名前を付けて選択アイテムを保存」で保存します。
  
* 読み込みは、「ファイル」⇒「ボディモーション」で行います。それぞれのBodyアイテムを選択した状態で対応するファイルの読み込みを行います。

* 読み込んだ動作データを再生するためには、それらを選択しておく必要があります。（複数物体がある場合は、全てのBodyMotionアイテムを選択）
