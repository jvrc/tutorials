...
ロボットにカメラが搭載されている場合、以下の形式でリストアップされます。
...
3. "Keep Relative Attitude"プロパティをtrueにしておくと、カメラの向きもロボットに追従します
...
3. 各BodyMotionアイテムを選択状態にして（まとめて選択してもOK)、[ファイル]-[名前を付けて選択アイテムを保存]で保存します。
...
1. シミュレーションを実行したChoreonoidを一旦終了し、再度Choreonoidを起動します。
  
   .. code-block:: bash

    $ choreonoid /usr/share/choreonoid-1.5/project/PA10Pickup.cnoid

2. アイテムビューで該当するBodyItemを選択した状態で対応するファイルの読み込みを行います。読み込みは、[ファイル]-[読み込み]-[ボディモーション]で行います。
...
2. [ツール]-[Movie Generator]を実行します。すると「動画用連番ファイルの作成」ダイアログが表示されます。
